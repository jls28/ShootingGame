...
void SoundManager::DuplicatePlay(const string & key, int volumePercent)
{
	if (IsBufIndex(key))
	{
		if (IsPlaying(key))
...
void SoundManager::DuplicateDistancePlay(const string & key, Vector3 position, int volumePercent)
{
	if (IsBufIndex(key))
	{
		if (IsPlaying(key))
